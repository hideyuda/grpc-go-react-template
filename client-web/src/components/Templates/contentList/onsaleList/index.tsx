...
import Card from "components/Templates/contentList/onsaleList/components/Card";
...
import { contentToolMaster } from "entities/Master/ContentTool";

function OnsaleList(): JSX.Element {
  ...
  useEffect(() => {
    const getContentList = async () => {
      ...
      if (user) {
        const req = new ContentUserIdRequest();
        req.setUserId(user?.id || 0);
        const res = await client.getRecommendedListByUser(req, {});
        res.getContentList().map((content) => {
          setContentList(contentList => [...contentList, content.toObject()]);
        });

      } else {
        const req = new google_protobuf_empty_pb.Empty
        const res = await client.getTrendList(req, {});
        res.getContentList().map((content) => {
          setContentList(contentList => [...contentList, content.toObject()]);
        });
      }
  ...
  const handleSearch = () => {
    ...
    console.log(keyword, toolList, categoryList);
    ...
    if (sort === "Newest") {
      ...
      req.setSortBy(ContentSearchRequest.SortBy.TOP);
    } else {
      req.setSortBy(ContentSearchRequest.SortBy.HOTTEST);
    }

      client.getListBySearch(req, {}, (err, res) => {
        if (err) {
          console.log(err);
        } else {
          setContentList([]);
        ...
        <CustomBox m={2}>
          ...
          <CustomBox mb={3} m={2}>
            ...
            <Autocomplete
              ...
              disableClearable={true}
              defaultValue={[]}
              options={contentToolMaster}
              renderInput={(params) => <CustomInput {...params} variant="standard" />}
              onChange={(event, value) => {
                event.preventDefault();
                setToolList(value.map((tool) => typeof tool === "string" ? parseInt(tool) : tool.value));
              }}
            />
          </CustomBox>
          <CustomBox mb={3} m={2}>
        ...
        >
          <CustomBox pb={3}>
            {contentList.map((content) => (
              <Link key={content.uuid} to={`/content?uuid=${content.uuid}`}>
                <Card
                  // image={meeting}
